...
def compose_mandopop(title: str = "晚风里的光", minutes: float = 3.0) -> Project:
    ...
    project.tracks = [
        Track(name="Piano", kind="instrument", channel=0, program=0, preset="SYSTEM/钢琴/piano", volume=90, effects=[Effect(type="eq", params={"bands": [{"type": "peaking", "frequency": 350, "gain_db": -2, "q": 1.0}]})]),
        Track(name="Bass", kind="instrument", channel=1, program=38, preset="SYSTEM/贝司/o3-bass", volume=86, effects=[Effect(type="compressor", params={"threshold_db": -16, "ratio": 3, "makeup_db": 1})]),
        Track(name="Pad", kind="instrument", channel=2, program=89, preset="SYSTEM/音垫/o3-pad", volume=74, effects=[Effect(type="reverb", params={"mix": 0.16, "decay": 0.35})]),
        Track(name="Lead Melody", kind="instrument", channel=3, program=81, preset="SYSTEM/合成器/o3-lead", volume=88, effects=[Effect(type="delay", params={"time": 0.24, "feedback": 0.18, "mix": 0.16})]),
        Track(name="Counter Melody", kind="instrument", channel=4, program=0, preset="SYSTEM/键盘乐器/keys", volume=76),
